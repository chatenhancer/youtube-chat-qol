...
const localesDir = path.join(rootDir, 'apps', 'docs', 'src', 'i18n');
const blogDir = path.join(rootDir, 'apps', 'docs', 'src', 'content', 'blog');
const baseLocale = 'en.json';
const basePath = path.join(localesDir, baseLocale);

type BlogFrontmatter = Record<string, string | string[]>;
...
function parseFrontmatter(source): BlogFrontmatter {
  ...
  for (const line of source.split('\n')) {
    ...
    if (listItem && currentKey) {
      ...
      (data[currentKey] as string[]).push(cleanFrontmatterValue(listItem[1]));
    }
  }
