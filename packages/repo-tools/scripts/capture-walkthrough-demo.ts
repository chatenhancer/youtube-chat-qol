...
} from './walkthrough-locales.ts';
import {
  configureWalkthroughProfileLocale,
  getWalkthroughProfilePath
} from './walkthrough-profile.ts';

const scriptDir = path.dirname(fileURLToPath(import.meta.url));
const repoRoot = path.resolve(scriptDir, '..', '..', '..');
const extensionDir = path.join(repoRoot, 'dist', 'extension-chrome');
const walkthroughAssetDir = path.resolve(scriptDir, '..', 'assets', 'walkthrough');
const walkthroughCursorDir = path.join(walkthroughAssetDir, 'cursors');
const walkthroughAudioDir = path.join(walkthroughAssetDir, 'audio');
const logoPath = path.join(repoRoot, 'apps', 'extension', 'src', 'assets', 'icons', 'icon.svg');
const avatarLogoPath = path.join(extensionDir, 'icons', 'icon-128.png');
const docsFontDir = path.join(repoRoot, 'apps', 'docs', 'public', 'assets', 'fonts');
...
const pointerCursorPath = path.join(walkthroughCursorDir, 'pointer.svg');
const handCursorPath = path.join(walkthroughCursorDir, 'hand.svg');
...
async function seedWalkthroughExtensionState(context, activeSourceUrl) {
  ...
  try {
    ...
    await extensionPage.evaluate(([
      ...
      return Promise.all([
        new Promise<void>((resolve) => chrome.storage.sync.set({
        ...
        new Promise<void>((resolve) => chrome.storage.local.remove([
          playgroundDisplayNameKey,
          playgroundIdentityKey
        ], resolve))
      ]);
...
async function installDemoPlaygroundBackend(context) {
  ...
  await serviceWorker.evaluate(() => {
    ...
    globalThis.WebSocket = DemoPlaygroundWebSocket as unknown as typeof WebSocket;
  });
}
  ...
  await page.evaluate(() => {
    ...
    const getOrCreateEffect = () => {
      ...
      if (!(effect instanceof HTMLDivElement)) {
        ...
        Object.assign((effect as HTMLElement).style, {
          inset: '0',
          overflow: 'visible',
          pointerEvents: 'none',
          position: 'fixed',
...
async function resolveWalkthroughSourceUrl(page) {
  const sourceCandidates = await page.evaluate(() => {
    ...
    return {
      canonicalUrl: (document.querySelector('link[rel="canonical"]') as HTMLLinkElement | null)?.href || '',
      currentUrl: window.location.href,
      openGraphUrl: (document.querySelector('meta[property="og:url"]') as HTMLMetaElement | null)?.content || '',
...
async function focusChatHeader(page, chat, recorder, options: any = {}) {
...
async function focusMessageArea(page, chat, recorder, options: any = {}) {
...
async function focusComposerArea(page, chat, recorder, options: any = {}) {
  const composer = chat.locator('yt-live-chat-message-input-renderer').first();
...
async function showComposerDraftResult(page, chat, recorder, durationMs, options: any = {}) {
...
async function typeIntoComposerHuman(chat, recorder, text, options: any = {}) {
  const input = getChatComposerInput(chat);
  await input.waitFor({ state: 'visible', timeout: 10_000 });
  await input.click();
    ...
    try {
      ...
      const debug = await chat.locator('body').evaluate((body, key) => {
        ...
        return {
          ...
          menuButtonContextWired: message instanceof HTMLElement
            ? (message.querySelector('#menu') as HTMLElement | null)?.dataset.ytcqContextWired || ''
            : ''
        };
...
async function openMessageMenuWithVisibleClick(page, chat, recorder, messageKey, options: any = {}) {
  await closeNativeMenus(chat);
  const source = await getDemoMessageSource(chat, messageKey, { center: false });
...
async function installLiveChatMask(chat, translationDemo) {
  await chat.locator('body').evaluate((body, { demoTranslation, textDirection }) => {
    ...
    const createDemoMessage = (entry, index) => {
      ...
      (message as HTMLElement & { data: unknown }).data = {
    ...
    const installDemoMessages = () => {
      ...
      const existing = new Set(
        Array.from(stage.querySelectorAll<HTMLElement>('.ytcq-demo-message')).map(
          (message) => message.dataset.ytcqDemoKey || ''
        )
      );
...
async function installDemoCursor(page) {
  ...
  await page.evaluate(([pointerSrc, handSrc, hotspot, position]) => {
    ...
    (cursor as HTMLElement).style.transform = `translate(${position.x - hotspot.x}px, ${position.y - hotspot.y}px)`;
...
async function createScreencastFrameRecorder(page) {
  ...
  const clickCues: any[] = [];
  const stageCues: any[] = [];
  const frameWaiters = new Set<(buffer: Buffer | null) => void>();
...
async function hoverWithCursor(page, locator, recorder, label, options: any = {}) {
...
async function clickWithCursor(page, locator, recorder, label, options: any = {}) {
...
async function setWalkthroughCaption(page, recorder, id, anchorBox = null, options: any = {}) {
...
async function showWalkthroughCaptionFor(page, recorder, id, options: any = {}) {
  const caption = getWalkthroughCaption(id);
...
function getWalkthroughClickCaption(id, options: any = {}) {
  const caption = getWalkthroughCaption(id);
...
async function showDemoCaptionFor(page, recorder, title, body, options: any = {}) {
  await fadeOutDemoCaptionAndFocus(page, recorder, 280);
...
async function setDemoCaption(page, title, body, anchorBox = null, options: any = {}) {
...
async function captureStableLocatorState(locator, recorder, label, options: any = {}) {
...
async function moveCursor(page, x, y, durationMs, recorder, options: any = {}) {
...
async function setDemoCameraForBox(page, recorder, box, options: any = {}) {
...
function getCameraTransformForBox(box, scale, options: any = {}) {
...
async function encodeCapturedVideo(
  frameCount,
  { clickCues = [], pipedVideoPath: inputVideoPath }: AudioCueOptions = {}
) {
...
async function muxPipedVideoToOutput(
  inputVideoPath,
  frameCount,
  { clickCues = [] }: AudioCueOptions = {}
) {
...
function appendAudioCueArgs(args, frameCount, { clickCues = [] }: AudioCueOptions = {}) {
...
function getAvailableAudioCueGroups({ clickCues = [] }: AudioCueOptions = {}) {
  return [
    {
      cues: clickCues,
