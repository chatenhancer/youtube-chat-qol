...
const extensionAssetsDir = path.join(extensionRoot, 'src', 'assets');
...
function getBuildOptions(target): BuildOptions {
  return {
    ...sharedBuildOptions,
    define: {
      ...sharedDefines,
...
async function buildTarget(target) {
  ...
  await Promise.all([
    ...
    build({
      ...getBuildOptions(target),
      entryPoints: [path.join(extensionRoot, 'src', 'youtube', 'chat-feed', 'page.ts')],
      outfile: path.join(extensionDir, 'chat-feed-page.js'),
      format: 'iife'
    }),
    build({
      ...getBuildOptions(target),
      entryPoints: [path.join(extensionRoot, 'src', 'features', 'lite-mode', 'bootstrap-entry.ts')],
  ...
  await Promise.all([
    ...
    writeMinifiedHtml(
      path.join(extensionRoot, 'src', 'onboarding.html'),
...
async function buildCssBundle(sources, { layered = false } = {}) {
  const chunks = [
    [
      ...
      ' *',
      ' * Edit the ordered source files under apps/extension/src/styles instead of this built file.',
      ' */'
    ].join('\n')
  ];
...
async function writeMinifiedHtml(
  infile,
  outfile,
  { inlineCss, stylesheetHref }: { inlineCss?: string | Promise<string>; stylesheetHref?: string } = {}
) {
