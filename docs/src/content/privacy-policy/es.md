...
- **Datos de aros de avatar:** se guardan usando el almacenamiento local de la extensión (`chrome.storage.local`). Incluyen el nombre del autor, la fecha y hora en que se añadió el aro, la URL del stream y, cuando están disponibles, la URL del avatar, el ID del canal y el título del stream de los usuarios a los que añades expresamente un aro desde su perfil de mensajes recientes. La selección sigue disponible entre streams en el perfil actual del navegador y solo se usa para decorar avatares coincidentes; no comprueba si un usuario está conectado.
