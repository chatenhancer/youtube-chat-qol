---
title: "Motywy już są!"
date: 2026-07-07
description: "Motywy czatu są już dostępne, zaczynając od Aero, żeby czat na żywo w YouTube był bardziej twój."
slug: "themes-are-here"
image: "./2026-07-07-themes-are-here-assets/fc7c094f3fbf6ac2.png"
cover_width: 84
locale: "pl"
translationKey: "themes-are-here"
tags: []
---

*Motywy czatu są już dostępne w wersji 0.17!*

Motywy mają sprawić, że czat będzie bardziej osobisty. Na razie będziemy udostępniać gotowy zestaw motywów (zaczynając od **Aero**), a w przyszłości pojawią się motywy z możliwością dostosowania.

:::media-left

![Motyw — Aero](./theme-setting-pl.webp){rotate=3.5deg}

Aby włączyć motyw, przejdź do sekcji **Wygląd** w ustawieniach rozszerzenia. Wybierz jeden z dostępnych motywów, żeby odświeżyć interfejs czatu!

:::

## O motywie Aero
Aero to motyw, który naśladuje estetykę interfejsów czatu z końca 2007 roku. Jest nostalgiczny, wodny i odświeżający! 💧

Pomysły na motywy wyślij na [hello@chatenhancer.com](mailto:hello@chatenhancer.com).
