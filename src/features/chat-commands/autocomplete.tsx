...
export function createCommandAutocomplete(
  options: ChatCommandAutocompleteOptions
  ...
  const render = (state: CommandAutocompleteState): void => {
    ...
    activeCard.append(
      el<HTMLDivElement>(
        <div class="ytcq-command-autocomplete-list" role="listbox">
          {state.suggestions.map((suggestion, index) => (
            <button
              type="button"
              class={[
                'ytcq-command-autocomplete-option',
                index === activeIndex ? 'ytcq-command-autocomplete-option-active' : ''
              ]
                .filter(Boolean)
                .join(' ')}
              data-ytcq-command-autocomplete-index={String(index)}
              role="option"
              aria-selected={String(index === activeIndex)}
            >
              <span class="ytcq-command-autocomplete-name">{suggestion.label}</span>
              <span class="ytcq-command-autocomplete-description">{suggestion.description}</span>
            </button>
          ))}
        </div>
      )
  ...
  const update = (): void => {
    ...
    activeIndex = key === activeKey ? Math.min(activeIndex, state.suggestions.length - 1) : 0;
    activeKey = key;
    render(state);
  };
  ...
  const handlePointerDown = (event: MouseEvent): void => {
    ...
    if (target instanceof Node && (activeCard.contains(target) || options.isFromChatInput(target)))
      return;
    close();
  };
...
function shouldHideExactAutocomplete(
  ...
  const exactSuggestions = suggestions.filter((suggestion) =>
    isExactSuggestion(suggestion, context)
  );
  ...
  if (context.kind === 'command') {
    ...
    return (
      exactSuggestions.length === suggestions.length &&
...
function isExactCommandSuggestion(
  suggestion: CommandAutocompleteSuggestion,
  fragment: string
): boolean {
...
function runsWithoutArgument(
  ...
  return Boolean(
    suggestion.command.runWithoutArgumentNames?.includes(context.fragment.toLowerCase())
  );
}
...
function createCommandAutocompleteCard(): HTMLElement {
  return el<HTMLElement>(
    <section class="ytcq-command-autocomplete-card" aria-label={t('chatCommands')} />
  );
}
