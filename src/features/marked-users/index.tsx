...
export function getMarkedUserIdentityFromParticipant(
  participant: HTMLElement
): MarkedUserIdentity | null {
...
export function applyMarkedUserRing(
...
export function cleanupStaleMarkedUsers(): void {
  ...
  document
    .querySelectorAll<HTMLElement>(
      '[data-ytcq-marked-user-key], .ytcq-markable-user-avatar, .ytcq-marked-user-avatar, .ytcq-marked-user-ring-host'
    )
    .forEach(clearMarkedUserRing);
  document
    .querySelectorAll<HTMLElement>('.ytcq-marked-user-ring-animation-host')
    .forEach((host) => {
...
function updateMarkedUserToggleButton(
...
function appendPositionedRingAnimation(
  host: HTMLElement,
  target: HTMLElement,
  overlay: HTMLElement
): void {
...
function removeExistingRingAnimations(
  target: HTMLElement,
  host = getNativeRingAnimationHost(target)
): void {
  const targetId = target.dataset.ytcqMarkedUserRingTargetId;
  target
    .querySelectorAll(':scope > .ytcq-marked-user-ring-animation')
    .forEach((element) => element.remove());
