...
function drawQuietPerimeter(context: CanvasRenderingContext2D, width: number, height: number): void {
  drawPerimeterStroke(context, width, height, 'rgba(62, 166, 255, 0.16)', 4);
}

function drawShimmerPerimeter(context: CanvasRenderingContext2D, width: number, height: number, progress: number): void {
  drawPerimeterStroke(context, width, height, createShimmerGradient(context, width, height, progress), 11);
}
...
function createShimmerGradient(
  ...
  const gradient = context.createConicGradient(progress * Math.PI * 2 - Math.PI * 0.8, width / 2, height / 2);
