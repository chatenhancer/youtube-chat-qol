...
import {
  INBOX_BUTTON_CLASS,
  INBOX_BUTTON_HAS_UNREAD_CLASS,
  INBOX_BUTTON_SELECTOR
} from './selectors';
...
function createInboxButton(options: InboxButtonOptions): HTMLButtonElement {
  const handleClick = (event: MouseEvent): void => {
    ...
    if (event.currentTarget instanceof HTMLElement) options.onToggle(event.currentTarget);
  };

  return el<HTMLButtonElement>(
    <button
      type="button"
      class={INBOX_BUTTON_CLASS}
      data-ytcq-inbox-owner={INBOX_BUTTON_OWNER_ID}
      title={t('inbox')}
      aria-label={getInboxAriaLabel(options.getUnreadCount())}
      onClickCapture={handleClick}
    >
      {createInboxIcon()}
      <span class="ytcq-inbox-badge" hidden />
    </button>
  );
}
...
export function refreshInboxSurfaces(getUnreadCount: () => number): void {
  document
    .querySelectorAll<HTMLButtonElement>(INBOX_BUTTON_SELECTOR)
