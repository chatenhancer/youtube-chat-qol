import type { PublicUserIdentity } from '../../../../shared/playground/protocol';
import type {
...
const SIDE_BLAST_MARGIN = 72;
const SIDE_POSITION_LIMIT = SIDE_BLAST_MARGIN + 90;
const TOP_BLAST_MARGIN = 170;
const BOTTOM_FALLOFF_MARGIN = 92;
const PLATFORM_HEIGHT = 10;
const PLATFORM_SIDE_MARGIN = 22;

export interface StickAroundFighter {
  attackUntil: number;
...
export interface StickAroundPlatform {
  height: number;
  kind: 'center' | 'side';
  width: number;
  x: number;
...
export interface StickAroundBubble {
  angle: number;
  height: number;
...
export interface StickAroundParticle {
  color: string;
  life: number;
  maxLife: number;
  size: number;
...
export function stepStickAroundSimulation(
  ...
  const dt = prepareSimulationClock(simulation, game, now);
  simulation.lastTime = now;
  simulation.frame += 1;
  ensureStickAroundFighters(simulation, game, now);
  if (simulation.roundSeed !== game.roundSeed) {
    simulation.roundSeed = game.roundSeed;
    simulation.platforms = createPlatforms(simulation.width, simulation.height, simulation.roundSeed);
  }
  spawnStickAroundHazards(simulation, game.hazards, messageTexts, now);

  const fighters = Object.values(simulation.fighters);
  fighters.forEach((fighter) => {
    stepFighter(simulation, fighter, normalizeControls(inputs[fighter.userId], now), dt, now);
  });

  maybeApplyFighterContact(simulation, fighters, now);
  stepBubbles(simulation, fighters, dt, now);
  stepParticles(simulation, dt);
  fighters.forEach((fighter) => maybeLoseStock(simulation, fighter, now));
...
function prepareSimulationClock(
  simulation: StickAroundSimulation,
  game: PublicStickAroundGame,
  now: number
): number {
  ...
  return clamp((now - simulation.lastTime) / 1000, 0, MAX_STEP_SECONDS);
}
...
function ensureStickAroundFighters(simulation: StickAroundSimulation, game: PublicStickAroundGame, now: number): void {
  ...
  players.forEach(({ player, role, x }) => {
    ...
    simulation.fighters[player.userId].label = player.displayName || 'Player';
  });
}
...
function spawnStickAroundHazards(
  ...
  hazards.forEach((hazard) => {
    ...
    const { height, width } = getBubbleSize(text, hazard.weight, simulation.width);
...
function getBubbleSize(
  ...
  const minWidth = Math.round(clamp(52 + weight * 2, 52, maxWidth));
  const rawContentWidth = Math.max(1, text.length * BUBBLE_APPROX_CHAR_WIDTH);
  const width = Math.round(clamp(rawContentWidth + BUBBLE_HORIZONTAL_PADDING, minWidth, maxWidth));
...
function maybeLoseStock(simulation: StickAroundSimulation, fighter: StickAroundFighter, now: number): void {
  ...
  loseStock(simulation, fighter, now);
}

function loseStock(simulation: StickAroundSimulation, fighter: StickAroundFighter, now: number): void {
  if (fighter.stocks <= 0) return;
  fighter.stocks -= 1;
  ...
  spawnParticles(
    simulation,
    clamp(fighter.x + FIGHTER_WIDTH / 2, 20, simulation.width - 20),
    clamp(fighter.y + FIGHTER_HEIGHT / 2, 20, simulation.height - 20),
