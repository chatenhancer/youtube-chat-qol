...
export function openReplayTriviaGamePanel(
  ...
  const canvas = el<HTMLCanvasElement>(
    <canvas
      class="ytcq-replay-trivia-canvas"
      aria-label={t('gamesReplayTrivia')}
      role="application"
      tabIndex={0}
    />
  );
  ...
  if (!context || !canRenderReplayTriviaCanvas(context)) {
    const fallback = el<HTMLDivElement>(
      <div class="ytcq-replay-trivia-game-fallback">{t('gamesReplayTriviaCanvasUnavailable')}</div>
    );
  ...
  canvas.addEventListener('mousemove', handleReplayTriviaCanvasMouseMove, {
    signal: listeners.signal
  });
  canvas.addEventListener('mouseleave', handleReplayTriviaCanvasMouseLeave, {
    signal: listeners.signal
  });

  void getReplayTriviaAssets()
    .then((assets) => {
...
function maybeGenerateReplayTriviaQuestions(state: ReplayTriviaPanelRuntime): void {
  ...
  if (
    ...
    state.generationToken.gameId !== state.game.gameId ||
    state.generationToken.expiresAt <= Date.now()
  ) {
...
function getReplayTriviaPreparationError(error: unknown): string {
  ...
  if (
    message.includes('Replay Trivia question generation returned an incomplete question pack.') ||
...
function handleReplayTriviaCanvasMouseMove(event: MouseEvent): void {
  ...
  const point = getCanvasPoint(state.canvas, event);
  if (state.phase === 'finished') {
    const hoveredCloseButton = Boolean(
      state.closeButtonRect && isPointInRect(point, state.closeButtonRect)
    );
  ...
  const hoveredAnswerIndex =
    state.hitboxes.find((hitbox) => isPointInRect(point, hitbox.rect))?.index ?? null;
...
function selectReplayTriviaAnswer(
  state: ReplayTriviaPanelRuntime,
...
function drawChatRound(
  ...
  const elapsed =
    state.phase === 'question' ? now - state.phaseStartedAt : Number.POSITIVE_INFINITY;
  const introProgress = getDelayedProgress(elapsed, CHAT_INTRO_DELAY_MS, CHAT_MESSAGE_ANIMATION_MS);
  const promptProgress = getDelayedProgress(
    elapsed,
    CHAT_PROMPT_DELAY_MS,
    CHAT_MESSAGE_ANIMATION_MS
  );
  ...
  if (introProgress > 0) {
    introHeight = drawAnimatedFriendBubble(
      context,
      question.friendIntro,
      28,
      introY,
      322,
      FRIEND_BUBBLE_MIN_HEIGHT,
      18,
      introProgress,
      {
  ...
  if (promptProgress > 0) {
    promptHeight = drawAnimatedFriendBubble(
      context,
      [
        {
          bold: true,
  ...
  context.fillText(
    `${state.currentQuestionIndex + 1}/${state.game.totalQuestions}`,
    CANVAS_WIDTH - 24,
    58 + offsetY
  );
...
function drawAnswerPicker(
  ...
  const answerLayouts = createAnswerOptionLayouts(
    context,
    question.answers,
    gridX,
    gridY,
  ...
  const gridBottom = answerLayouts.reduce(
    (bottom, answer) => Math.max(bottom, answer.rect.y + answer.rect.height),
    gridY
  );
  ...
  drawInputBar(
    context,
    state.userAnswerIndex === null
      ? t('gamesReplayTriviaWaitingForResponse')
      : t('gamesReplayTriviaLockedIn'),
    offsetY
  );
}
...
function drawSelectedAnswerTarget(
  ...
  const size = 31;
  context.drawImage(state.assets.target, selected.rect.x + 6, selected.rect.y + 6, size, size);
}
...
function drawRevealedAnswers(
  ...
  const userAnswer =
    state.userAnswerIndex === null
      ? t('gamesReplayTriviaNoAnswer')
      : question.answers[state.userAnswerIndex];
  const opponentAnswer =
    state.opponentAnswerIndex === null
      ? t('gamesReplayTriviaNoAnswer')
      : question.answers[state.opponentAnswerIndex];
  const userCorrect =
    question.correctIndex !== undefined && state.userAnswerIndex === question.correctIndex;
  const opponentCorrect =
    question.correctIndex !== undefined && state.opponentAnswerIndex === question.correctIndex;
  ...
  const correctAnswer =
    question.correctIndex === undefined ? '' : question.answers[question.correctIndex] || '';
  const replyProgress =
    state.phase === 'reveal'
  ...
  if (replyProgress > 0) {
    drawAnimatedFriendBubble(
      context,
      boldFriendReplyAnswer(reply, correctAnswer),
      28,
      300 + offsetY,
      376,
      FRIEND_BUBBLE_MIN_HEIGHT,
...
function drawScoreModal(
  ...
  drawScoreTitle(
    context,
    title,
    getOpponentDisplayName(state),
    modal.x + modal.width / 2,
    modal.y + 42
  );
  const progress = easeOutCubic(
    Math.min(1, Math.max(0, (now - state.phaseStartedAt) / SCORE_FLAP_ANIMATION_MS))
  );
...
function drawFinalModal(
  ...
  if (isTie) {
    ...
    if (state.assets.tie) {
      ...
      drawFinalStampFallback(
        context,
        t('gamesReplayTriviaStampOkay'),
        modal.x + 62,
        modal.y + 118,
        '#ffd400'
      );
    ...
    if (state.assets.bestie) {
      ...
      drawFinalStampFallback(
        context,
        t('gamesReplayTriviaStampBestie'),
        modal.x + 62,
        modal.y + 118,
        '#00df32'
      );
    ...
    if (state.assets.blocked) {
      ...
      drawFinalStampFallback(
        context,
        t('gamesReplayTriviaStampBlocked'),
        modal.x + 62,
        modal.y + 118,
        '#ff1616'
      );
...
function drawStampSmoke(
  ...
  const puffs = [
    ...
    { delay: 0.05, radius: 18, startX: 0.4, startY: -0.12, travelX: 44, travelY: -14 },
    { delay: 0.1, radius: 16, startX: -0.36, startY: 0.3, travelX: -48, travelY: 20 },
    { delay: 0.15, radius: 20, startX: 0.34, startY: 0.26, travelX: 50, travelY: 22 },
    { delay: 0.2, radius: 13, startX: -0.06, startY: 0.42, travelX: -8, travelY: 34 }
  ];
...
function drawFriendBubble(
  ...
  minHeight: number,
  fontSize: number,
  { flipImage = false, image, kind, tail = true }: FriendBubbleOptions
): number {
  ...
  const lines = wrapFriendBubbleText(
    context,
    text,
...
function boldFriendReplyAnswer(
  reply: string,
  correctAnswer: string
): string | FriendBubbleTextSegment[] {
...
function findFriendReplyAnswerMatch(reply: string, answer: string, startIndex: number): number {
  ...
  while (index >= 0) {
    ...
    if (isFriendReplyAnswerBoundary(reply, before) && isFriendReplyAnswerBoundary(reply, after))
      return index;
...
function wrapFriendBubbleText(
  ...
  words.forEach((word) => {
    const candidate = current.length > 0 ? [...current, { text: ' ' }, word] : [word];
...
function toFriendBubbleWords(
  text: string | readonly FriendBubbleTextSegment[]
): FriendBubbleTextSegment[] {
...
function drawPlayerBubble(
  ...
  if (image) {
    drawBubbleSprite(
      ...
      width,
      height,
      side === 'left' ? 'left-tail' : 'right-tail',
      false
    );
  } else {
    context.fillStyle = color;
...
function drawBubbleSpriteSlices(
  ...
  const sourceLeft =
    kind === 'left-tail' ? Math.min(sourceWidth * 0.34, sourceHeight * 0.72) : sourceHeight / 2;
  const sourceRight =
    kind === 'right-tail' ? Math.min(sourceWidth * 0.34, sourceHeight * 0.72) : sourceHeight / 2;
...
function measureAnswerText(
  ...
  const height = Math.ceil(
    Math.max(ANSWER_MIN_HEIGHT, lines.length * lineHeight + ANSWER_TEXT_INSET_Y * 2)
  );
  return {
    height,
    text: {
...
function fitFontSize(
  ...
  for (let size = preferred; size >= minimum; size -= 1) {
    ...
    if (
      wrapText(context, text, maxWidth, maxLines).every(
