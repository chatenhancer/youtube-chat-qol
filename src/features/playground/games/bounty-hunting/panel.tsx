...
const BOUNTY_HUNTING_CLAIM_INDICATOR_CLASS = 'ytcq-bounty-hunting-claim-indicator';
const BOUNTY_HUNTING_CLAIM_INDICATOR_AVATAR_CLASS = 'ytcq-bounty-hunting-claim-indicator-avatar';
const BOUNTY_HUNTING_CLAIM_INDICATOR_AMOUNT_CLASS = 'ytcq-bounty-hunting-claim-indicator-amount';
...
export function openBountyHuntingGamePanel(
  ...
  const canvas = el<HTMLCanvasElement>(
    <canvas
      class="ytcq-bounty-hunting-canvas"
      aria-label={t('gamesBountyHunting')}
      role="application"
      tabIndex={0}
    />
  );
  ...
  if (!context || !canRenderBountyHuntingCanvas(context)) {
    const fallback = el<HTMLDivElement>(
      <div class="ytcq-bounty-hunting-game-fallback">
        {t('gamesBountyHuntingCanvasUnavailable')}
      </div>
    );
  ...
  const runtime: BountyHuntingPanelRuntime = {
    ...
    canvas,
    claimSoundIndex: 0,
    claimedMessageIndicators: new Set(),
    closePanel,
    compactMode: false,
    context,
    currentUserId,
    ...
    roundStartDivider: null,
    roundStartDividerHost: null,
    roundStartDividerPlacementFrame: null,
  ...
  canvas.addEventListener('mouseleave', handleBountyHuntingCanvasMouseLeave, {
    signal: listeners.signal
  });
  canvas.addEventListener('keydown', handleBountyHuntingCanvasKeydown, {
    signal: listeners.signal
  });
...
export function updateBountyHuntingGamePanel(
  game: PublicBountyHuntingGame,
  currentUserId: string
): void {
  const runtime = activeBountyHuntingPanel;
...
function scheduleBountyHuntingPreparationCheck(
  ...
  runtime.preparationTimer = window.setTimeout(() => {
    ...
    if (
      ...
      countBountyHuntingObservedCandidateTypes([...runtime.preparationMessages.values()]) <
        BOUNTY_HUNTING_BOUNTY_COUNT
    ) {
...
function maybeSendBountyHuntingWitness(
  ...
  queueBountyHuntingWitness(runtime, message.messageId, bountyIds, message.messageTimestampUsec);
}
...
function queueBountyHuntingWitness(
  ...
  const existing = runtime.pendingWitnesses.get(messageId) || {
    bountyIds: new Set<string>()
  };
  bountyIds.forEach((bountyId) => existing.bountyIds.add(bountyId));
  runtime.pendingWitnesses.set(messageId, {
...
function flushBountyHuntingWitnesses(runtime: BountyHuntingPanelRuntime): void {
  ...
  for (const [messageId, witness] of runtime.pendingWitnesses) {
    const observation: { bountyIds: string[]; messageId: string; messageTimestampUsec?: string } = {
      bountyIds: [...witness.bountyIds],
      messageId
    };
    if (witness.messageTimestampUsec)
      observation.messageTimestampUsec = witness.messageTimestampUsec;
...
function scheduleBountyHuntingRoundStartDividerPlacement(runtime: BountyHuntingPanelRuntime): void {
  if (!hasBountyHuntingRoundStartBoundary(runtime)) return;
  if (runtime.roundStartDividerPlacementFrame !== null) return;
  runtime.roundStartDividerPlacementFrame = scheduleFrame(() => {
    runtime.roundStartDividerPlacementFrame = null;
    if (!activeBountyHuntingPanel || activeBountyHuntingPanel.canvas !== runtime.canvas) return;
    if (runtime.game.status !== 'active') return;
    ensureBountyHuntingRoundStartDivider(runtime);
  });
...
function maybeClaimBountyHuntingBounty(
  ...
  const payload: { bountyId: string; messageId: string; messageTimestampUsec?: string } = {
    bountyId: bounty.id,
    messageId: message.messageId
  };
  if (message.messageTimestampUsec) payload.messageTimestampUsec = message.messageTimestampUsec;
  runtime.onAction(runtime.game.gameId, 'claimBounty', payload);
}
...
function playBountyHuntingClaimFeedback(runtime: BountyHuntingPanelRuntime): void {
  const soundPath =
    BOUNTY_HUNTING_CLAIM_SOUND_PATHS[
      runtime.claimSoundIndex % BOUNTY_HUNTING_CLAIM_SOUND_PATHS.length
    ];
...
function syncBountyHuntingClaimIndicators(runtime: BountyHuntingPanelRuntime): void {
  const currentClaimKeys = new Set(
    runtime.game.bounties
      .map((bounty) => bounty.claim)
      .filter((claim): claim is BountyHuntingClaim => Boolean(claim))
      .map(getBountyHuntingClaimKey)
  );

  for (const indicator of [...runtime.claimedMessageIndicators]) {
    if (
      currentClaimKeys.has(indicator.dataset.ytcqBountyHuntingClaimKey || '') &&
      indicator.isConnected
    )
      continue;
    removeBountyHuntingClaimIndicator(runtime, indicator);
  }

  runtime.game.bounties.forEach((bounty) => {
    if (!bounty.claim) return;
    const message = findBountyHuntingChatMessageById(bounty.claim.messageId);
    if (message) showBountyHuntingClaimIndicator(runtime, bounty.claim, message);
  });
}

function showBountyHuntingClaimIndicatorsForMessage(
  runtime: BountyHuntingPanelRuntime,
  message: HTMLElement
): void {
  const messageId = getMessageStableId(message);
  if (!messageId) return;
  runtime.game.bounties.forEach((bounty) => {
    if (bounty.claim?.messageId === messageId)
      showBountyHuntingClaimIndicator(runtime, bounty.claim, message);
  });
}

function showBountyHuntingClaimIndicator(
  runtime: BountyHuntingPanelRuntime,
  claim: BountyHuntingClaim,
  message: HTMLElement
): void {
  const claimKey = getBountyHuntingClaimKey(claim);
  const existing = findBountyHuntingClaimIndicator(message, claimKey);
  if (existing) {
    runtime.claimedMessageIndicators.add(existing);
    return;
  }

  const host = getBountyHuntingClaimIndicatorHost(message);
  const indicator = createBountyHuntingClaimIndicator(runtime, claim);
  host.append(indicator);
  runtime.claimedMessageIndicators.add(indicator);
}

function createBountyHuntingClaimIndicator(
  runtime: BountyHuntingPanelRuntime,
  claim: BountyHuntingClaim
): HTMLElement {
  ...
  const indicator = el<HTMLSpanElement>(
    <span
      class={BOUNTY_HUNTING_CLAIM_INDICATOR_CLASS}
      data-ytcq-bounty-hunting-claim-key={getBountyHuntingClaimKey(claim)}
      aria-label={label}
      role="status"
      title={label}
    >
      <span
        class={BOUNTY_HUNTING_CLAIM_INDICATOR_AVATAR_CLASS}
        style={{
      ...
      <span class={BOUNTY_HUNTING_CLAIM_INDICATOR_AMOUNT_CLASS}>
...
function findBountyHuntingClaimIndicator(
  message: HTMLElement,
  claimKey: string
): HTMLElement | null {
  return (
    [...message.querySelectorAll<HTMLElement>(`.${BOUNTY_HUNTING_CLAIM_INDICATOR_CLASS}`)].find(
      (indicator) => indicator.dataset.ytcqBountyHuntingClaimKey === claimKey
    ) || null
  );
}

function getBountyHuntingClaimIndicatorHost(message: HTMLElement): HTMLElement {
  const messageContainer = message.querySelector<HTMLElement>('#message-container');
  if (messageContainer) return messageContainer;
  const messageText = message.querySelector<HTMLElement>('#message');
  return messageText?.parentElement || message;
}

function clearBountyHuntingClaimIndicators(runtime: BountyHuntingPanelRuntime): void {
  [...runtime.claimedMessageIndicators].forEach((indicator) =>
    removeBountyHuntingClaimIndicator(runtime, indicator)
  );
}

function removeBountyHuntingClaimIndicator(
  runtime: BountyHuntingPanelRuntime,
  indicator: HTMLElement
): void {
  runtime.claimedMessageIndicators.delete(indicator);
  indicator.remove();
}
...
function handleBountyHuntingCanvasKeydown(event: KeyboardEvent): void {
  ...
  const runtime = activeBountyHuntingPanel;
  if (!runtime) return;
  const hitbox = runtime.hitboxes.find(
    (candidate) => candidate.action === 'ready' || candidate.action === 'close'
  );
  if (!hitbox) return;
  ...
  handleBountyHuntingCanvasClick(
...
function getBountyHuntingCanvasAction(
  runtime: BountyHuntingPanelRuntime,
  event: MouseEvent
): 'close' | 'ready' | null {
...
function startBountyHuntingLoop(): void {
  ...
  const tick = (now: number): void => {
    ...
    if (!active) return;
    active.pixelRatio = syncBountyHuntingCanvasPixelRatio(
      active.canvas,
      active.pixelRatio,
      active.compactMode
    );
...
function maybeSendBountyHuntingTimerActions(runtime: BountyHuntingPanelRuntime): void {
  if (runtime.game.status === 'countdown' && !runtime.startRoundSent) {
    if (Date.now() - runtime.game.phaseStartedAt >= BOUNTY_HUNTING_COUNTDOWN_MS) {
      ...
      sendBountyHuntingStartRound(runtime);
    }
  }
...
function drawBountyHuntingCompactStatus(runtime: BountyHuntingPanelRuntime, now: number): void {
  ...
  if (game.status === 'countdown') {
    ...
    drawCenteredFittedText(
      context,
      tBountyHuntingBarnum('gamesBountyHuntingStarting'),
      224,
      49,
      94,
      {
  ...
  if (game.status === 'preparing') {
    drawCenteredFittedText(
      context,
      tBountyHuntingBartle('gamesBountyHuntingLoadingStatus'),
      224,
      32,
      116,
      {
  ...
  if (game.status === 'roundOver') {
    drawCenteredFittedText(
      context,
      tBountyHuntingBarnum('gamesBountyHuntingRoundOver'),
      224,
      32,
      150,
      {
  ...
  drawCenteredFittedText(
    context,
    tBountyHuntingBarnum('gamesBountyHuntingTimeRemaining'),
    224,
    46,
    122,
    {
...
function drawBountyHuntingCompactReadyButton(
  runtime: BountyHuntingPanelRuntime,
  now: number
): void {
  const { assets, context } = runtime;
  ...
  if (image) {
    context.drawImage(
      ...
      COMPACT_READY_BUTTON_RECT.y,
      COMPACT_READY_BUTTON_RECT.width,
      COMPACT_READY_BUTTON_RECT.height
    );
  } else {
    drawRoundedRect(
      context,
  ...
  if (flashAmount > 0)
    drawBountyHuntingButtonFlash(context, COMPACT_READY_BUTTON_RECT, flashAmount, image);
...
function drawBountyHuntingCompactBounties(runtime: BountyHuntingPanelRuntime): void {
  ...
  displayBounties.forEach((bounty, index) =>
    drawBountyHuntingCompactBounty(runtime, bounty, index)
  );
}
...
function drawBountyHuntingCompactBounty(
  ...
  drawFittedText(context, getBountyHuntingCompactBountyLabel(bounty), x + 47, y + 14, 76, 10);
  ...
  if (bounty.claim) {
    drawBountyHuntingAvatar(
      runtime,
      bounty.claim.role,
      x + COMPACT_BOUNTY_CHIP_WIDTH - 8,
      y + COMPACT_BOUNTY_CHIP_HEIGHT - 4,
      7
    );
  }
}
...
function drawBountyHuntingLoading(runtime: BountyHuntingPanelRuntime, now: number): void {
  ...
  drawCenteredFittedText(
    context,
    t('gamesBountyHuntingLoading'),
    224,
    398 + LOADING_SCREEN_OFFSET_Y,
    220,
    {
      color: '#444',
...
function drawBountyHuntingWanted(runtime: BountyHuntingPanelRuntime, now: number): void {
  ...
  displayBounties.forEach((bounty, index) =>
    drawBountyHuntingBountyClaimAvatar(runtime, bounty, index)
  );
...
function drawBountyHuntingLedger(runtime: BountyHuntingPanelRuntime): void {
  ...
  drawBountyHuntingActionButton(runtime, tBountyHuntingBartle('gamesBountyHuntingClose'), 'close', {
...
function drawBountyHuntingTitle(
  ...
  if (assets.titleDecorLeft)
    context.drawImage(assets.titleDecorLeft, leftDecorX, decorY, decorWidth, 44);
  if (assets.titleDecorRight)
    context.drawImage(assets.titleDecorRight, rightDecorX, decorY, decorWidth, 44);
...
function drawBountyHuntingLiveScore(runtime: BountyHuntingPanelRuntime, now: number): void {
  ...
  drawBountyHuntingMoneyText(
    context,
    game.scores[opponentRole] || 0,
    LIVE_SCORE_LEFT_TEXT_X,
    LIVE_SCORE_MONEY_Y + offsetY,
    {
      align: 'left',
...
function getBountyHuntingReadyButtonFlashAmount(
  runtime: BountyHuntingPanelRuntime,
  now: number
): number {
...
function getBountyHuntingTimerStartPulseAmount(
  runtime: BountyHuntingPanelRuntime,
  now: number
): number {
...
function hasNewBountyHuntingReadyPlayer(
  ...
  return BOUNTY_HUNTING_PLAYER_ROLES.some(
    (role) => !previousGame.readyPlayers[role] && Boolean(nextGame.readyPlayers[role])
  );
}
...
function drawBountyHuntingRankedAvatar(
  ...
  runtime.context.fillText(
    String(rank),
    x - LEDGER_RANK_TEXT_X_OFFSET,
    y - LEDGER_RANK_TEXT_Y_OFFSET
  );
}
...
function drawBountyHuntingDivider(runtime: BountyHuntingPanelRuntime, y: number): void {
  if (runtime.assets.divider) {
    runtime.context.drawImage(
      runtime.assets.divider,
      LEDGER_DIVIDER_X,
      y,
      LEDGER_DIVIDER_WIDTH,
      10
    );
    return;
  }
...
function drawFittedTextBlock(
  ...
  while (
    ...
    (lines.length > options.maxLines ||
      lines.some((line) => getCanvasTextWidth(context, line, font) > maxWidth))
  ) {
    fontSize -= 1;
  ...
  visibleLines.forEach((line, index) => {
    const fittedLine =
      getCanvasTextWidth(context, line, font) <= maxWidth
        ? line
...
function getFittedCanvasText(
  ...
  return {
    font: fittedFont,
    text:
      getCanvasTextWidth(context, text, fittedFont) > maxWidth
...
function getBountyHuntingWinnerLabel(runtime: BountyHuntingPanelRuntime): string {
  ...
  if (runtime.game.winnerUserId) {
    label =
      runtime.game.winnerUserId === runtime.currentUserId
        ? tBountyHuntingBarnum('gamesBountyHuntingYou')
        : tBountyHuntingBarnum('gamesBountyHuntingThem');
  }
...
function getBountyHuntingBountyDescription(bounty: PublicBountyHuntingBounty): string {
  if (bounty.descriptionKey && BOUNTY_HUNTING_DESCRIPTION_KEYS.has(bounty.descriptionKey)) {
...
function getBountyHuntingRoleClaimCount(
  game: PublicBountyHuntingGame,
  role: BountyHuntingPlayerRole
): number {
...
function getBountyHuntingOpponentLabel(
  game: PublicBountyHuntingGame,
  currentUserId: string
): string {
...
function getBountyHuntingCanvasPoint(
  canvas: HTMLCanvasElement,
  event: MouseEvent
): { x: number; y: number } {
