...
export function createGamePanelShell({
  ...
  const panel = el<HTMLElement>(
    ...
    >
      ...
      >
        <span class={`${classNamePrefix}-icon`}>{icon}</span>
        <div class={`${classNamePrefix}-title-wrap`}>
          <div
            ref={(element: HTMLDivElement) => (titleElement = element)}
            class={`${classNamePrefix}-title`}
          >
            {title}
          </div>
          <div
            ref={(element: HTMLDivElement) => (subtitleElement = element)}
            class={`${classNamePrefix}-subtitle`}
          >
            {subtitle}
          </div>
        </div>
        <button
          ref={(element: HTMLButtonElement) => (compactButton = element)}
          type="button"
          class={`ytcq-game-panel-compact-toggle ${classNamePrefix}-compact-toggle`}
          hidden
        />
        <button
          ref={(element: HTMLButtonElement) => (closeButton = element)}
          type="button"
          class={`ytcq-game-panel-close ${classNamePrefix}-close`}
      ...
      <div ref={(element: HTMLDivElement) => (body = element)} class={`${classNamePrefix}-body`}>
...
function animateGamePanelPositionChange(panel: HTMLElement, applyPosition: () => void): void {
  ...
  const animation = panel.animate([{ transform: startTransform }, { transform: finalTransform }], {
