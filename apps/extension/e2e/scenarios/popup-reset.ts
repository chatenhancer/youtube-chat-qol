...
async function resetExtensionFromPopup(context: BrowserContext): Promise<void> {
  await test.step('Confirm popup reset', async () => {
    ...
    try {
      ...
      await expect(popup.locator('.popup-reset-dialog-message')).toHaveCSS('font-family', 'Inter, Arial, sans-serif');
      await expect(popup.locator('.popup-reset-dialog-confirm')).toHaveCSS('font-family', 'Inter, Arial, sans-serif');
      await expect(popup.locator('.popup-reset-dialog-list li')).toHaveCount(10);
